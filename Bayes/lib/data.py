...
# DATA FUNCTIONS
# =============================================================================
def _load_image(file_path='/data/hook/feature_activations.npy', csv_path='/data/train.csv', is_torch=True, is_filtred=False):
    ...
    if is_filtred:
        ...
        return np.array(image_names), np.array(activations), np.array(y), np.array(indices)
    else:
        csv.field_size_limit(sys.maxsize)
...
def get_dataset(file_path, csv_path, seed=35, return_names = False, is_filtred=False):
    ...
    # Load image
    if is_filtred:
        image_names, X, y, indices = _load_image(file_path,csv_path,is_filtred=is_filtred)
        X = X[indices]
        ...
        X_train, X_temp, y_train, y_temp, names_train, names_temp = train_test_split(
            X, y, image_names, train_size=7724, random_state=seed)

        X_val, X_test, y_val, y_test, names_val, names_test = train_test_split(
            X_temp, y_temp, names_temp, train_size=1446, random_state=seed)
...
def save_csv_uncertainty(images_names, y_test, uncertainty, thresholds, output_dir="filtered_uncertainty"):
    """
...
def load_top_k_least_uncertain_samples(csv_path, data_path, y, names, k=750, num_classes=6):
    """
    Carga las K muestras con menor incertidumbre por clase.

    Parámetros:
    -----------
    csv_path : str
        Ruta al CSV con columnas ['filename', 'class', 'uncertainty']
    data_path : str
        Ruta donde se encuentran los archivos .npy
    k : int
        Número de muestras con menor incertidumbre a seleccionar por clase
    num_classes : int o None
         Número total de clases (opcional, solo se usa para validacion)
    Retorna:
    --------
    X_tensor : torch.Tensor
        Tensores de características (float32)
    y_tensor : torch.Tensor
        Tensores de etiquetas (long)
    """

    uncertainty_df = pd.read_csv(csv_path, delimiter=",")
    uncertainty_map = dict(zip(uncertainty_df["Image Name"], uncertainty_df["Uncertainty"]))

    X = np.load(data_path)

    thresholds = calcular_thresholds_por_clase(csv_path,k)
    print(thresholds)
    #thresholds = {0: np.float64(0.0337282675038537), 4: np.float64(0.1004824550993903), 1: np.float64(0.0440683187900549), 
        #3: np.float64(0.1638707915969968), 5: np.float64(0.1083554306179882), 2: np.float64(0.1429370914603632)}
    X_train_filtrado, y_train_filtrado, names_train_filtrado = filtrar_muestras_por_threshold(
        X, y, names, uncertainty_map, thresholds)

    return np.array(X_train_filtrado), np.array(y_train_filtrado)


def calcular_thresholds_por_clase(csv_path, k=750):
    df = pd.read_csv(csv_path)
    thresholds = {}

    for clase in df['Grado_ISUP'].unique():
        df_clase = df[df['Grado_ISUP'] == clase]
        df_clase_ordenado = df_clase.sort_values(by='Uncertainty')
        
        if len(df_clase_ordenado) >= k:
            threshold = df_clase_ordenado.iloc[k-1]["Uncertainty"]
        else:
            threshold = df_clase_ordenado.iloc[-1]["Uncertainty"]  # máximo disponible
        
        thresholds[int(clase)] = threshold
    
    return thresholds

def filtrar_muestras_por_threshold(X, y, names, uncertainty_map, thresholds):
    filtered_indices = []
    tam_clase = np.zeros(6)

    for i, name in enumerate(names):
        clase = y[i]
        incertidumbre = uncertainty_map.get(name, float("inf"))
        if incertidumbre <= thresholds.get(clase, float("inf")):
            filtered_indices.append(i)
            tam_clase [clase] +=1

    for i in range(6):
        print(f"Clase {i}: {tam_clase[i]}") 

    X_filtrado = X[filtered_indices]
    y_filtrado = y[filtered_indices]
    names_filtrado = [names[i] for i in filtered_indices]

    return X_filtrado, y_filtrado, names_filtrado
