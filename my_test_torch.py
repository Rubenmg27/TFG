...
if '.' in __name__:
    ...
    from lib.plot import (plot_class_uncertainty, plot_reliability_diagram,
                          plot_accuracy_vs_uncertainty, plot_model_accuracy,
                          plot_confusion_matrix, plot_uncertainty_distribution, plot_uncertainty_matrix, plot_threshold_uncertainty)
...
def test(epochs, verbose=False):
    ...
    for name, dataset in datasets.items():
        ...
        (reliability_data[name],
         ...
         uncertainty_fn, uncertainty_fp, uncertainty_correct, uncertainty) = predict(model, X_test_tensor, y_test, num_classes, samples=passes,verbose= True, images_names=image_names)
        ...
        plot_threshold_uncertainty (y_pred_mean, y_test, uncertainty, classes, output_dir)
        # Liberate model
        del model
    ...
    plot_accuracy_vs_uncertainty(output_dir, acc_data, px_data, w, h, colours, threshold=0.1)
