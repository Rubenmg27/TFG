...
parser.add_argument('--split_part', type=int, default=1, help="Parte del dataset a procesar")
...
data_dir = '/data'
df_train = pd.read_csv(os.path.join(data_dir, 'train_prueba.csv')) # train  train_prueba
...
#df_parts = np.array_split(df,22)
#df = df_parts[args.split_part - 1]
...
class enetv3(nn.Module):
    def __init__(self, modelname, out_dim=5, freeze_bn=True):
        ...
        self.hook_handle = self.basemodel._conv_head.register_forward_hook(self.save_activation)

    def save_activation(self, module, input, output):
        self.feature_activations = input[0].detach().cpu().numpy()
...
def load_models(model_files):
    ...
    for model_f in model_files:
        ...
        model.load_state_dict(torch.load(model_f),strict=False)
        model.eval()
        model.to(device)
        models.append(model)
...
with torch.no_grad():
    for data in tqdm(loader):
        ...
        for i, tta_model in enumerate(tta_models2):
            ...
            feature_activations_list.append(tta_model.model.feature_activations)
        LOGITS2.append(logits / num_models)

    save_1 = np.array(feature_activations_list)
    ...
    save_2 = np.array(feature_activations_list)
    save = (save_1 + save_2) / 2

    feature_activations_array = np.array(save)
    np.save(f"/home/ruben/TFGRuben/Data/hook/activations/feature_activations_prueba.npy", feature_activations_array)
    print("Activaciones guardadas en feature_activations.npy")
    print(f"Forma de las activaciones",feature_activations_array.shape)

LOGITS = (torch.cat(LOGITS2).cpu() + torch.cat(LOGITS22).cpu()) / 2
print(f"Shape of LOGITS after concatenation and averaging: {LOGITS.shape}")  # Debug: tamaño de LOGITS después de concatenar y promediar
...
df[['image_id', 'isup_grade']].to_csv('submission_prueba.csv', index=False)
print(df)
print()
