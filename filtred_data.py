...
def predict(model, X_test, y_test, num_classes, samples=100, verbose=False, images_names=None):
    ...
    save_csv_uncertainty(images_names,y_test, uncertainty, thresholds=[0.9, 0.85, 0.8])
...
def test(epochs, verbose=False):
    ...
    for name, dataset in datasets.items():
        ...
        # ---------------------------------------------------------------------
        # Get dataset
        #X_train, y_train, _, _, _, _,image_names, _, _ = get_dataset(file_path=args.data_path, csv_path=args.csv_path, seed=None, return_names = True)
        ...
        (reliability_data[name],
         ...
         uncertainty_fn, uncertainty_fp, uncertainty_correct, uncertainty) = predict(model, X_train_tensor, y_train, num_classes, samples=passes,verbose= True, images_names=image_names)
