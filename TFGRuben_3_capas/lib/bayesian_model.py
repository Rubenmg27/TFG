...
class BayesianENet(nn.Module):
    def __init__(self, modelo, in_features, output_dim=5, hidden_dim=128):
        ...
        self._project_conv = bnn.BayesConv2d(in_channels=in_features,out_channels=320, kernel_size=1, stride=1, padding=0, bias=False,prior_mu=0, prior_sigma=0.1)
        self.bn1 = nn.BatchNorm2d(320, eps=0.001, momentum=0.010000000000000009, affine=True, track_running_stats=True)
        self._conv_head = bnn.BayesConv2d(in_channels=320,out_channels=1280, kernel_size=1, stride=1, padding=0, bias=False,prior_mu=0, prior_sigma=0.1)
        #self._conv_head = Conv2dStaticSamePadding(320, 1280, kernel_size=(1, 1), stride=(1, 1), bias=False, image_size = 48)
        self.bn2 = nn.BatchNorm2d(1280, eps=0.001, momentum=0.010000000000000009, affine=True, track_running_stats=True)
        self.pooling = nn.AdaptiveAvgPool2d(output_size=1)
        self.dropout_p = nn.Dropout(p=0.2, inplace=False)
    ...
    def load_pretrained_weights(self, path_pretrained_myfc_weights='/data/hook/myfc_weights.npy',
                                path_pretrained_myfc_bias='/data/hook/myfc_bias.npy', path_pretrained_conv_head_weights='../TFGRuben/Data/hook/conv_head_weights.npy',
                                path_pretrained_project_conv_weights='./Data/hook/project_conv_weights.npy'):
        ...
        pretrained_project_conv_weights = np.load(path_pretrained_project_conv_weights)
        ...
        with torch.no_grad():
            ...
            self._project_conv.weight_mu.copy(torch.tensor(pretrained_project_conv_weights))
