...
if '.' in __name__:
    ...
    from lib.data import get_dataset, get_filtered_dataset, cargar_dataset
...
def test(epochs, verbose=False):
    ...
    for name, dataset in datasets.items():
        ...
        test_files = [os.path.join(args.data_path, f"feature_activations{n}.npy") for n in [39, 40, 41, 42, 43, 44]]
        activations = np.load(test_files[0])
        X_test = np.array(activations)
        y_test = y[(10616-(X_test.shape[0]*6)):]
    ...
    # GROUPED PLOTS
    # -------------------------------------------------------------------------
    plot_reliability_diagram(output_dir, reliability_data, w, h, colours,threshold =None)
